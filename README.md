...
|-------|------|-------------|
| 1 | Triage | Resolve and unshallow the checkout, pull `master`, adopt any open PR, check the fork against upstream, and **scan** for candidate work — the fork has Issues disabled, so the backlog is what the cycle writes down, not a tracker |
...
- **Issues are disabled on the fork.** Triage is a scan, `Closes #N` is meaningless, and the deferred-candidate list lives in the PR body.
